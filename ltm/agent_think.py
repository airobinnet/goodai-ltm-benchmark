...
from utils.llm import (make_user_message, make_assistant_message, make_system_message,
                       LLMContext, ask_llm, count_tokens_for_model)
...
class LTMAgent:
    ...
    def reply(self, user_content: str, cost_callback: Callable[[float], Any] = None) -> str:
        ...
        response = self._completion(context, temperature=self.config.llm_temperature, label="reply",
                                    cost_callback=cost_callback)
        response = self._reflect(context, response, cost_callback=cost_callback)
    ...
    def _completion(self, context: List[dict[str, str]], temperature: float, label: str,
                    cost_callback: Callable[[float], Any], max_tokens: int = None) -> str:
        max_tokens = max_tokens or self.max_completion_tokens
        response_text = ask_llm(
            context, self.model, temperature=temperature, timeout=self.config.timeout, max_response_tokens=max_tokens,
            cost_callback=cost_callback, max_overall_tokens=self.max_prompt_size,
        )
    ...
    def _debug_actions(self, context: list[dict[str, str]], temperature: float, response_text: str):
        ...
        # Remove previous data if it's a new conversation
        save_dir = _debug_dir.joinpath(self.save_name)
        if self.session.message_count == 0 and save_dir.exists():
